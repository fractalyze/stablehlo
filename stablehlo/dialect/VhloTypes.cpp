...
namespace mlir::vhlo
{
  ...
  void VhloTypeConverter::addBuiltinToVhloConversions()
  {
    ...
    addConversion(
        [&](IndexType type)
        { return IndexV1Type::get(type.getContext()); });
    addConversion(
        [&](IntegerType type)
        { return convertBuiltinIntegerType(type); });
    addConversion(
        [&](NoneType type)
        { return NoneV1Type::get(type.getContext()); });
  ...
  namespace
  {
    ...
    ParseResult parseTypeArray(AsmParser &parser, SmallVector<Type> &typeArray)
    {
      ...
      auto parseEle = [&]()
      { return parser.parseType(typeArray.emplace_back()); };
...
#include "stablehlo/dialect/VhloTypeDefs.cpp.inc" // NOLINT(build/include)
        >(context);
  }
