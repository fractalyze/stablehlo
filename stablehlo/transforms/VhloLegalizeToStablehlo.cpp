...
Attribute convertDotDimensionNumbers(OpType vhloOp,
                                     const TypeConverter *typeConverter) {
...
LogicalResult implodeSpecial(const OpConversionPattern<VhloOpTy> &pattern,
  ...
  // TODO(chokobole): Implement this. Dependency: DynamicGatherOpV1
  ...
  // TODO(chokobole): Implement this. Dependency: RecvOpV1, SendOpV1
  return success();
}
...
SpecialResult
convertDenseI64Array(const TypeConverter *typeConverter, StringAttr vhloName,
                     Attribute vhloAttr,
                     SmallVector<NamedAttribute> &stablehloAttrs) {
...
LogicalResult removeDefaults(const OpConversionPattern<VhloOpTy> &pattern,
  ...
  // TODO(chokobole): Implement this. Dependency: InfeedOpV1
  // TODO(chokobole): Implement this. Dependency: OutfeedOpV1
  // TODO(chokobole): Implement this. Dependency: RecvOpV1, SendOpV1
  if constexpr (std::is_same<VhloOpTy, vhlo::ReduceWindowOpV1>::value) {
...
public:
  ...
  LogicalResult
  matchAndRewrite(VhloOpTy vhloOp, typename VhloOpTy::Adaptor adaptor,
    ...
    SmallVector<NamedAttribute> stablehloAttrs;
    if (failed(removeDefaults(*this, vhloOp, vhloAttrs)))
      return failure();
